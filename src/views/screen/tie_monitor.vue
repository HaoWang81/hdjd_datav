...
// 默认选中的项目
const defaultCheckedItems = ["今日制芯", "今日合箱", "今日开箱", "本周合箱", "月度制芯"];
...
onMounted(() => {
  ...
  if (userAgent.match(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i)) {
    isMobile.value = true;
    // checkList.value =  ["今日制芯", "今日合箱", "今日开箱", "本周合箱", "月度制芯"]
...
const fetchData = () => {
  axios.post("/api/screen/tie_monitor/").then((response) => {
...
const selectChange = (value) => {
  if (value == '造型工段') {
    ...
    checkList.value = ["今日转序", "本周转序", "月度转序", "月度销售", "毛坯成品"]
  } else if (value == '打磨工序') {
    checkList.value = ["打磨", "热处理", "精修", "毛坯检验", "涂装", "毛坯在制"]
  } else if (value == '加工工段') {
    checkList.value = ["今日加工", "本周加工", "月度加工", "加工月度销售", "加工在制"]
  } else if (value == '油漆工段') {
